...
int verifyParam(const char* p, const char* type){
...
int callArg(ListManager* lstMgr, char* name, void* val){
	...
	if(arg->hasParam){
		...
		if (arg->asInt)
		{
			...
			if (arg->defParam != NULL){
				if (val == NULL){
					arg->function((char*) arg->defParam, NULL);
				}
				else{
				...
				arg->function((char*)val, NULL);
			}
		}
	}
	else{
		arg->function(NULL, NULL);
	}
...
int parseArgs(ListManager* lstMgr, int argc, char* argv[]){
	...
	for (i = 1; i < argc; ++i){
		...
		if(val->hasParam){
			...
			val->function(val->defParam, NULL);
		}
		else{
			//fprintf(stderr, "==== NO PARAM\n");
			val->function(NULL, NULL);
		}
